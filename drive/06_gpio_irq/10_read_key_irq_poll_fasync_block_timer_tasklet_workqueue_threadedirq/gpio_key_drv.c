...
{
    struct gpio_desc *gpiod_key = data;
    int val;
   
    val = gpiod_get_value(gpiod_key);
    printk("gpio_key_thread_func: the process is %s pid %d\n",current->comm, current->pid);	
	printk("gpio_key_thread_func key %d %d\n", desc_to_gpio(gpiod_key), val);
...
static int gpio_key_probe(struct platform_device *pdev)
{
    ...
    err = request_threaded_irq(irq, gpio_key_isr, gpio_key_thread_func, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING, "pi_gpio_key", key_gpio);
